/******************************************************************************\
*  client/src/BindStatRelation.cpp                                             *
...
void BindStatRelation::multiRelationColumn(const QString& col_arg)
{
	QString col = col_arg;
	...
	QString alias = QString("%1_%2").arg(mRelationForeignTable).arg(col);
...
BindStatRelation::BindStatRelation(QObject *obj_parent) :
...
void BindStatRelation::setTextColumn(const QString& col)
{
	mColumns.clear();
	mRelationColumns.clear();

	multiRelationColumn(col);
}

QString BindStatRelation::textColumn() const
{
	return mColumns.first();
}

QStringList BindStatRelation::columns() const
{
...
QVariantList BindStatRelation::customViewActions(int id) const
{
	// Add in the ID
	...
	action["user_data"] = QString("stat_relation_%1_%2_%3").arg(
		mTable).arg(mOtherTable).arg(mCurrentID);

	return QVariantList() << action << refreshItemCache();
}

QVariantList BindStatRelation::customUpdateActions(int id) const
{
...
QVariantList BindStatRelation::customDeleteActions(int id) const
{
	QVariantMap action;
	action["action"] = "delete";
	action["table"] = mOtherTable;
...
QWidget* BindStatRelation::viewer() const
{
	return mViewer;
}

void BindStatRelation::setViewer(QWidget *view)
{
	Q_ASSERT( view->inherits("QListWidget") );

	mViewer = qobject_cast<QListWidget*>(view);
	mViewer->setSelectionMode(QListWidget::NoSelection);
}

QWidget* BindStatRelation::editor() const
{
	return mEditor;
}

void BindStatRelation::setEditor(QWidget *edit)
{
...
void BindStatRelation::handleViewResponse(const QVariantMap& values)
{
	Q_UNUSED(values);
...
void BindStatRelation::retrieveUpdateData(QVariantMap& row)
{
	Q_UNUSED(row);
...
QString BindStatRelation::relationID() const
{
	return mRelationID;
}

QString BindStatRelation::relationForeignTable() const
{
	return mRelationForeignTable;
}

QString BindStatRelation::relationForeignID() const
{
	return mRelationForeignID;
}

void BindStatRelation::setRelation(const QString& id,
...
QString BindStatRelation::extraColumn() const
{
	return mExtraColumn;
}

void BindStatRelation::setExtraColumn(const QString& col)
{
	mExtraColumn = col;
}

QString BindStatRelation::extraPrompt() const
{
	return mExtraPrompt;
}

void BindStatRelation::setExtraPrompt(const QString& prompt)
{
	mExtraPrompt = prompt;
}

QString BindStatRelation::table() const
{
	return mTable;
}

void BindStatRelation::setTable(const QString& tbl)
{
	mTable = tbl;
}

QString BindStatRelation::otherTable() const
{
	return mOtherTable;
}

void BindStatRelation::setOtherTable(const QString& tbl)
{
	mOtherTable = tbl;
}

void BindStatRelation::ajaxResponse(const QVariantMap& resp,
	const QString& user_data)
{
	QString user_data2 = QString("stat_relation_%1_%2_cache").arg(
	...
	if(user_data == user_data2)
	{
		...
		while( it.hasNext() )
		{
			...
			name.replace(QRegExp("\\$\\{extra\\:([^\\}]*)\\}"), QString());
			name.replace("${extra}", QString());
	...
	user_data2 = QString("stat_relation_%1_%2_%3").arg(
		mTable).arg(mOtherTable).arg(mCurrentID);
	...
	while( it.hasNext() )
	{
		...
		QString text = row.value( mColumns.first() ).toString();

		QString replace;

		int extra = row.value(mExtraColumn).toInt();
		if(extra < 0)
			replace = tr("-%1").arg(extra * -1);
		else
			replace = tr("+%1").arg(extra);

		text.replace(QRegExp("\\$\\{extra\\:([^\\}]*)\\}"), replace + "\\1");
		text.replace("${extra}", replace);
...
void BindStatRelation::selectionChanged()
{
	if(!mEditor)
		return;
...
void BindStatRelation::addSelectionChanged()
{
	if(!mAddList)
		return;
...
QPushButton* BindStatRelation::removeButton()
{
	return mRemoveButton;
}

void BindStatRelation::setRemoveButton(QPushButton *button)
{
	if(mRemoveButton)
...
QPushButton* BindStatRelation::editButton()
{
	return mEditButton;
}

void BindStatRelation::setEditButton(QPushButton *button)
{
	if(mEditButton)
...
QPushButton* BindStatRelation::addButton()
{
	return mAddButton;
}

void BindStatRelation::setAddButton(QPushButton *button)
{
	if(mAddButton)
...
QPushButton* BindStatRelation::cancelButton()
{
	return mCancelButton;
}

void BindStatRelation::setCancelButton(QPushButton *button)
{
	if(mCancelButton)
...
QPushButton* BindStatRelation::newButton()
{
	return mNewButton;
}

void BindStatRelation::setNewButton(QPushButton *button)
{
	if(mNewButton)
...
QStackedWidget* BindStatRelation::addStack()
{
	return mAddStack;
}

void BindStatRelation::setAddStack(QStackedWidget *stack)
{
	mAddStack = stack;
}

QListWidget* BindStatRelation::addList()
{
	return mAddList;
}

void BindStatRelation::setAddList(QListWidget *list)
{
	if(mAddList)
	{
...
SearchEdit* BindStatRelation::quickSearch()
{
	return mQuickSearch;
}

void BindStatRelation::setQuickSearch(SearchEdit *edit)
{
	if(mQuickSearch)
...
void BindStatRelation::queueRemoval()
{
	if(!mEditor)
		return;
...
void BindStatRelation::performEdit()
{
	if(!mEditor)
		return;
	...
	// Update the label
	QString text = data.value( mColumns.first() ).toString();

	QString replace;
	if(value < 0)
		replace = tr("-%1").arg(value * -1);
	else
		replace = tr("+%1").arg(value);
...
void BindStatRelation::startAdd()
{
...
void BindStatRelation::cancelAdd()
{
	if(!mEditor || !mAddStack)
		return;
...
void BindStatRelation::performAdd()
{
	Q_ASSERT(mEditor);
...
int BindStatRelation::promptForExtra(int default_value)
{
	if(!mEditor)
	...
	int extra = QInputDialog::getInteger(0, tr("Data Entry"), mExtraPrompt,
		default_value, -500, 500, 1, &ok);

	return ok ? extra : default_value;
}

QVariant BindStatRelation::refreshItemCache() const
{
	QVariantList cols;
...
void BindStatRelation::filterAddList()
{
...
bool BindStatRelation::itemMatches(const QVariantMap& map,
...
void BindStatRelation::addRequested()
{
